...
class Surface():
  def __init__(self, path, x_seed=None):
    ...
    self.tuple_path = [tuple(
            ...
            ( int(np.round(self.path[i,0],0)), int(np.round(self.path[i,1],0)) )
    )
  ...
  for sample_interval_x in sample_intervals:
    ...
    for x_seed in seeds:
      ...
      for ind, peak in enumerate(peaks):
        ...
        path_x_flip, path_y_flip = runge_kutta_4(x0, y0, h, x_seed, vector_array * -1, num_decimals=num_decimals)

        path_y_flip = path_y_flip[(path_x_flip > 0) & (path_x_flip < seismic_slice.shape[1])]
        path_x_flip = path_x_flip[(path_x_flip > 0) & (path_x_flip < seismic_slice.shape[1])]

        path_x_flip = path_x_flip[(path_y_flip > 0) & (path_y_flip < seismic_slice.shape[0])]
        path_y_flip = path_y_flip[(path_y_flip > 0) & (path_y_flip < seismic_slice.shape[0])]


        # Get the streamline
        path_x, path_y = runge_kutta_4(x0, y0, h, seismic_slice.shape[1] - x_seed, vector_array, num_decimals=num_decimals)


        path_y = path_y[(path_x > 0) & (path_x < seismic_slice.shape[1])]
        path_x = path_x[(path_x > 0) & (path_x < seismic_slice.shape[1])]

        path_x = path_x[(path_y > 0) & (path_y < seismic_slice.shape[0])]
        path_y = path_y[(path_y > 0) & (path_y < seismic_slice.shape[0])]
        ...
        if len(path) > 0 and len(path_flip) > 0:
            merged_path = np.concatenate((list(reversed(path_flip)), path))
...
def generate_surfaces_2D(seismic_line, num_surfaces, sample_interval_x, overlapp_threshold, mode):
    ...
    for surface in surfaces_seismic:
        surface.mean_heat = np.mean(heatmap[surface.path[:,0].astype(int),surface.path[:,1].astype(int)])

    #sort on mean heat
    surfaces_seismic_sorted  = list(reversed(sorted(surfaces_seismic, key=lambda obj: obj.mean_heat)))
    ...
    for i in range(num_surfaces):
        surface = overlapping_paths[i]
        surface_array[surface.path[:,0].astype(int), surface.path[:,1].astype(int)] = 1
...
sample_interval_x = 100 #the distance between traces to sample peaks/troughs from
overlapp_threshold = 25 #overlap tolerance for surfaces. Removes surfaces that have a higher overlap than 25 %
num_surfaces = 10 #number of top surfaces to plot
mode = 'both' #if we are picking on both peak and trough in each trace




surface_array = generate_surfaces_2D(seismic_line, num_surfaces=num_surfaces, overlapp_threshold=overlapp_threshold, sample_interval_x=sample_interval_x, mode=mode)

#fig,ax = plt.subplots(1,1,figsize=(20,20))
#ax.imshow(surface_array,cmap='gray')
#plt.show()
